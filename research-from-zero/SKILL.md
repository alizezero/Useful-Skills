...
5. **Initial Main-Text/SI Split**
   ...
   - When main claims and figures are mostly fixed, build an SI Evidence Pack: SI inventory, source-data cards, manuscript-number traceability, and provenance separation.
...
## SI Evidence Pack Rules

- Treat SI as evidence architecture, not storage for unused artifacts.
- Build SI in main-text evidence order, not experiment chronology.
- For every claim-carrying main figure, create a source-data card.
- For every important manuscript number, assign a source table, CSV, log, or reproducible script.
- Keep internal filenames, checkpoints, run names, dataset nicknames, and paths out of narrative prose except in reproducibility/provenance tables.
- Use `references/supplementary-information.md` for SI Note templates, source-data cards, manuscript-number traceability, and final SI audit.
